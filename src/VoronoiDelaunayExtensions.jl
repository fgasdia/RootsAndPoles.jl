...
Base.getindex(p::IndexablePoint2D)::Int64 = p._index
Base.setindex!(p::IndexablePoint2D, v::Int64) = setfield!(p, :_index, v)
Base.:+(p1::IndexablePoint2D, p2::IndexablePoint2D) = IndexablePoint2D(getx(p1)+getx(p2), gety(p1)+gety(p2), -1)
Base.:/(p1::IndexablePoint2D, n::Number) = IndexablePoint2D(getx(p1)/n, gety(p1)/n, -1)
